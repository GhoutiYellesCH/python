...
def solve_task_assignment():
    durations = [[9, 2, 7, 8], [6, 4, 3, 7], [5, 8, 1, 8], [7, 6, 9, 4]]
    ...
    worker_times = [
        model.NewIntVar(0, 20, f"worker_{i}_time") for i in range(num_workers)
    ]

    for i in range(num_workers):
        model.Add(
            worker_times[i] == sum(x[i, j] * durations[i][j] for j in range(num_tasks))
        )
        ...
        for i in range(num_workers):
            for j in range(num_tasks):
                if solver.Value(x[i, j]) == 1:
                    print(
                        f"Ouvrier {chr(65 + i)} réalise la tâche J{j + 1} en {durations[i][j]} heures"
                    )
