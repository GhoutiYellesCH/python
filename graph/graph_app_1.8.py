...
class GraphApp(App):
    def build(self):
        ...
        self.open_webview_button = Button(text="Open WebView")
        self.open_webview_button.bind(on_press=self.open_existing_webview)
        layout.add_widget(self.open_webview_button)
    ...
    def load_existing_graph(self):
        self.html_file = "graph.html"
        if os.path.exists(self.html_file):
            self.open_webview(self.html_file)

    def generate_and_load_graph(self, instance):
        self.html_file = self.generate_graph()
        self.open_webview(self.html_file)

    def open_existing_webview(self, instance):
        if hasattr(self, 'html_file') and os.path.exists(self.html_file):
            self.open_webview(self.html_file)

    def open_webview(self, html_file):
        # Run the WebView in a separate process
        process = multiprocessing.Process(target=self.start_webview, args=(html_file,))
        process.start()

    def start_webview(self, html_file):
...
def show_graph(graph, output_file="graph.html"):
    net = Network(notebook=True, cdn_resources='in_line',height="750px", width="100%", bgcolor="#222222", font_color="white", select_menu=True)
    net.from_nx(graph)
